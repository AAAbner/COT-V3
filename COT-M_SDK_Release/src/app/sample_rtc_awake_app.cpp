...
static uint8_t g_sleep_pram = NoneSave;
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
uint16_t sample_app_processevent(uint8_t task_id, uint16_t events)
{
    ...
    if ( events & SMAPLE_MSG_CONFIG_EVENT )
    {
        ...
        if( p_msg )
        {
			...
			osal_set_event(task_id, SMAPLE_MSG_TX_EVENT);
			#endif
	...
	{
		uint8_t data_h[255];
		memset(data_h,0x0,255);
		//hzx_readbuffer_test(0x0,data_h,255);
	...
	if(events & SMAPLE_MSG_WAKE_EVENT )
	{
		if( (g_sleep_pram & SaveDataRam ) == SaveDataRam )
			osal_set_event(task_id,SMAPLE_MSG_TX_EVENT);
		else
			osal_set_event(task_id,SMAPLE_MSG_CONFIG_EVENT);
		return (events ^ SMAPLE_MSG_WAKE_EVENT);
	}
	#endif
	if ( events & SMAPLE_MSG_SLEEP_EVENT )
    {
        ...
        if( p_msg )
        {
			g_sleep_pram = SaveDataBuffer | SaveDataRam ;
            p_msg->hdr.event = CMD_MAC_LAYER;
