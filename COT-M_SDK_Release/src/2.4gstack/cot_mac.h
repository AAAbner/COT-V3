...
#define MAC_EVENT_RANG_CMD          0x05
...
typedef enum
{
	UNCONFIRM            = 0x0c,    //unconfirm packages in sending   		不确定的包的发送
    CONFIRM              = 0x0d     //confirm packages in sending				确定的包的发送
}pkg_type_t;
...
void readbuffer_test( uint8_t offset, uint8_t *buffer, uint8_t size );
